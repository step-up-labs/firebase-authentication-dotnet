...
if ($preview) {
	$suffix = "preview-$(date -format yyyyMMddHHmmss)"
	write "Creating preview packages with version suffix $suffix"
	dotnet pack --configuration release --output $output -p:version=$suffix .\src\Auth\Auth.csproj
	dotnet pack --configuration release --output $output -p:version=$suffix .\src\Auth.UI.WPF\Auth.UI.WPF.csproj
